...
public class StatsReporter {
    ...
    public static void print(SourceStats s, boolean skipBlanks, boolean skipComments,
        ...
        log.accept("+----------+--------+------------+------------+--------+----------+");
        log.accept(String.format("| %-8s | %6s | %10s | %10s | %6s | %8s |",
                "Ext", "Files", "Lines", "Chars", "Blank", "Comment"));
        log.accept("+----------+--------+------------+------------+--------+----------+");

        for (Map.Entry<String, long[]> e : s.getByExtension().entrySet()) {
            long[] v = e.getValue();
            log.accept(String.format("| %-8s | %6d | %10d | %10d | %6d | %8d |",
                    e.getKey(), v[0], v[1], v[2], v[3], v[4]));
        }

        log.accept("+----------+--------+------------+------------+--------+----------+");
        log.accept(String.format("| %-8s | %6d | %10d | %10d | %6d | %8d |",
                "TOTAL", s.getTotalFiles(), s.getTotalLines(),
                s.getTotalChars(), s.getTotalBlanks(), s.getTotalComments()));
        log.accept("+----------+--------+------------+------------+--------+----------+");
